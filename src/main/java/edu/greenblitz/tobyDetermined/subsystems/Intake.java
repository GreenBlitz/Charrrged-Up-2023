...
public class Intake { //todo separate to two files
	...
	public void moveRoller(boolean reversed) {
		moveRoller(reversed ? RobotMap.Intake.REVERSE_POWER : RobotMap.Intake.POWER);
	}
	...
	private class Roller extends IntakeSubsystem {
		...
		private Roller() {
			rollerMotor = new WPI_TalonSRX(RobotMap.Intake.Motors.ROLLER_PORT);
			rollerMotor.setInverted(RobotMap.Intake.Motors.IS_REVERSED);
	...
	public class Extender extends IntakeSubsystem {
		...
		private Extender() {
			extender = new DoubleSolenoid(RobotMap.Pneumatics.PCM.PCM_ID, RobotMap.Pneumatics.PCM.PCM_TYPE, RobotMap.Intake.Solenoid.FORWARD_PORT, RobotMap.Intake.Solenoid.REVERSE_PORT);
		}
