...
import static edu.greenblitz.tobyDetermined.RobotMap.TelescopicArm.Extender.SWITCH_TYPE;

public class ExtenderSub extends GBSubsystem {
    private static ExtenderSub instance;
    private static ExtenderSub.ExtenderSubState state = ExtenderSub.ExtenderSubState.IN_ROBOT_BELLY_LENGTH;
    ...
    public static ExtenderSub getInstance() {
        init();
        return instance;
    }
    ...
    public static ExtenderSub.ExtenderSubState getHypotheticalState(double lengthInMeters) {
        if (lengthInMeters < ExtenderSub.ExtenderSubState.REVERSE_OUT_OF_BOUNDS.maxLength) {
            return ExtenderSub.ExtenderSubState.REVERSE_OUT_OF_BOUNDS;
        } else if (lengthInMeters < ExtenderSub.ExtenderSubState.IN_WALL_LENGTH.maxLength) {
            return ExtenderSub.ExtenderSubState.IN_WALL_LENGTH;
        } else if (lengthInMeters < ExtenderSub.ExtenderSubState.IN_ROBOT_BELLY_LENGTH.maxLength) {
            return ExtenderSub.ExtenderSubState.IN_ROBOT_BELLY_LENGTH;
        } else if(lengthInMeters < ExtenderSub.ExtenderSubState.OPEN.maxLength){
            return ExtenderSub.ExtenderSubState.OPEN;
        } else{
            return ExtenderSub.ExtenderSubState.FORWARD_OUT_OF_BOUNDS;
        }
    }
    ...
    public double getLegalGoalLength(double wantedLength){
        ...
        if (getHypotheticalState(wantedLength) == ExtenderSub.ExtenderSubState.FORWARD_OUT_OF_BOUNDS) {
        ...
        } else if (getHypotheticalState(wantedLength) == ExtenderSub.ExtenderSubState.REVERSE_OUT_OF_BOUNDS) {
            Console.log("OUT OF BOUNDS", "arm Extender is trying to move OUT OF BOUNDS" );
        ...
        else if (ElbowSub.getInstance().getState() == ElbowSub.ElbowState.IN_BELLY && getHypotheticalState(wantedLength).longerThan( ExtenderSub.ExtenderSubState.IN_ROBOT_BELLY_LENGTH)) {
            return (RobotMap.TelescopicArm.Extender.MAX_LENGTH_IN_ROBOT - RobotMap.TelescopicArm.Extender.LENGTH_TOLERANCE);
        }else if (ElbowSub.getInstance().getState() == ElbowSub.ElbowState.WALL_ZONE && getHypotheticalState(wantedLength).longerThan(ExtenderSub.ExtenderSubState.IN_WALL_LENGTH)) {
    ...
    public ExtenderSub.ExtenderSubState getState() {
        return state;
    }
    ...
    public enum ExtenderSubState{
        ...
        ExtenderSubState(double maxLength){
        ...
        public boolean longerThan(ExtenderSub.ExtenderSubState other){
            return this.maxLength > other.maxLength;
        }

        public boolean shorterOrEqualTo(ExtenderSub.ExtenderSubState other){
