...
public class ElbowSub extends GBSubsystem {
    ...
    public ElbowSubState state = ElbowSubState.IN_BELLY;
    ...
    public double getLegalGoalAngle(double angleInRads){
        ...
        if (getHypotheticalState(angleInRads) == ElbowSub.ElbowSubState.FORWARD_OUT_OF_BOUNDS || getHypotheticalState(angleInRads) == ElbowSub.ElbowSubState.BACKWARD_OUT_OF_BOUNDS){
        ...
        else if (getState() == ElbowSub.ElbowSubState.WALL_ZONE && Extender.getInstance().getState() != Extender.ExtenderState.IN_WALL_LENGTH) {
            ...
            return (state == ElbowSubState.IN_BELLY ? RobotMap.TelescopicArm.Elbow.STARTING_WALL_ZONE_ANGLE - RobotMap.TelescopicArm.Elbow.ANGLE_TOLERANCE : RobotMap.TelescopicArm.Elbow.END_WALL_ZONE_ANGLE+ RobotMap.TelescopicArm.Elbow.ANGLE_TOLERANCE);
        }else {
            return (angleInRads);
        }
...
//        return motor.getAbsoluteEncoder(SparkMaxAbsoluteEncoder.Type.kDutyCycle).getPosition();
    ...
    public ElbowSub.ElbowSubState getState() {
        return state;
    }
    ...
    public static ElbowSubState getHypotheticalState(double angleInRads) {
        if (angleInRads < ElbowSub.ElbowSubState.BACKWARD_OUT_OF_BOUNDS.maxAngle){
            return ElbowSubState.BACKWARD_OUT_OF_BOUNDS;
        }
        else if (angleInRads < ElbowSubState.IN_BELLY.maxAngle){
            return ElbowSubState.IN_BELLY;
        }
        else if (angleInRads < ElbowSubState.WALL_ZONE.maxAngle){
            return ElbowSubState.WALL_ZONE;
        }
        else if(angleInRads < ElbowSubState.OUT_ROBOT.maxAngle){
            return ElbowSubState.OUT_ROBOT;
        }
        return ElbowSubState.FORWARD_OUT_OF_BOUNDS;
    }
    ...
    public boolean isInTheSameState(double wantedAng) {
        return getHypotheticalState(getAngleRadians()) == getHypotheticalState(wantedAng) && (getHypotheticalState(wantedAng) != ElbowSub.ElbowSubState.FORWARD_OUT_OF_BOUNDS || getHypotheticalState(wantedAng) != ElbowSub.ElbowSubState.BACKWARD_OUT_OF_BOUNDS);
    }
    ...
    public enum ElbowSubState {
        ...
        private ElbowSubState(double maxAngle){
            this.maxAngle = maxAngle;
        }

        public boolean largerThen(ElbowSub.ElbowSubState checkState){
            return maxAngle > checkState.maxAngle;
        }

        public boolean smallerThen(ElbowSub.ElbowSubState checkState){
            return maxAngle < checkState.maxAngle;
        }
        public boolean smallerOrEqualTo(ElbowSub.ElbowSubState checkState){
            return maxAngle <= checkState.maxAngle;
        }
    }
