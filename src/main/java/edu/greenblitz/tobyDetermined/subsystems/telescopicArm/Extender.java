...
public class Extender extends GBSubsystem {
    ...
    public static ExtenderState getHypotheticalState(double lengthInMeters) {
        ...
        } else if (lengthInMeters > RobotMap.TelescopicArm.Extender.IN_FLOOR_MIN) {
            return ExtenderState.FLOOR_ZONE;
        } else {
    ...
    public void moveTowardsLength(double lengthInMeters) {
        ...
        if (Elbow.getInstance().getState() == Elbow.ElbowState.IN_BELLY && getHypotheticalState(lengthInMeters) == ExtenderState.OPEN) {
            ...
            // arm is gonna hit floor, move to safe length
        } else if (getHypotheticalState(lengthInMeters) == ExtenderState.FLOOR_ZONE && Elbow.getInstance().getState() == Elbow.ElbowState.FLOOR_ZONE) {
            setLengthByPID(RobotMap.TelescopicArm.Extender.SAFE_FLOOR_LENGTH);
        } else {
    ...
    public enum ExtenderState {
        ...
        IN_WALL_LENGTH,
        //the state corresponding to FLOOR_ZONE
        FLOOR_ZONE,
