...
public class NodeBase  {
    ...
    public NodeBase(){
      ...
      for (int j = 0; j < poses.length; j++) {
        ...
        nodeMap.put(PRE_INTAKE_GRAB_POSITION, new NodeArm(-0.02,0.196));
        ...
        nodeMap.get(ZIG_HAIL).addNeighbors(new PresetPositions[] {CONE_MID, CUBE_HIGH, LOW, CUBE_MID, CONE_HIGH, PRE_INTAKE_GRAB_POSITION, PRE_CONE_DROP});
        nodeMap.get(INTAKE_GRAB_CONE_POSITION).addNeighbors(new PresetPositions[] {PRE_INTAKE_GRAB_POSITION,INTAKE_GRAB_CUBE_POSITION});
        nodeMap.get(INTAKE_GRAB_CUBE_POSITION).addNeighbors(new PresetPositions[] {PRE_INTAKE_GRAB_POSITION,INTAKE_GRAB_CONE_POSITION});
        nodeMap.get(PRE_INTAKE_GRAB_POSITION).addNeighbors(new PresetPositions[] {ZIG_HAIL,INTAKE_GRAB_CONE_POSITION,INTAKE_GRAB_CUBE_POSITION,PRE_CONE_DROP});
        nodeMap.get(PRE_CONE_DROP).addNeighbors(new PresetPositions[] {POST_CONE_DROP});
        nodeMap.get(POST_CONE_DROP).addNeighbors(new PresetPositions[] {PRE_INTAKE_GRAB_POSITION,INTAKE_GRAB_CONE_POSITION,INTAKE_GRAB_CUBE_POSITION});

        currentNode = INTAKE_GRAB_CONE_POSITION;

        nodeMap.get(CONE_HIGH).setClawPos(3);
        nodeMap.get(CONE_MID).setClawPos(3);
        nodeMap.get(CUBE_HIGH).setClawPos(3);
        nodeMap.get(CUBE_MID).setClawPos(3);
        nodeMap.get(INTAKE_GRAB_CONE_POSITION).setClawPos(1);
        nodeMap.get(INTAKE_GRAB_CUBE_POSITION).setClawPos(2);
        nodeMap.get(PRE_CONE_DROP).setClawPos(1);
        nodeMap.get(POST_CONE_DROP).setClawPos(1);
        nodeMap.get(LOW).setClawPos(3);
